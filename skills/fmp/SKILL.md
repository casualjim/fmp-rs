...
description: >
  ...
  sector/industry performance. Also trigger when the user wants full-market bulk datasets
  (all company profiles, all EOD prices for a date, bulk financial statements, ratings,
  or scores for every symbol at once) — use the `bulk` namespace for these. The fmp
  binary reads FMP_API_KEY from the environment.
metadata:
  version: "1.0.0"
  binary: fmp
...
- **Bulk datasets**: full-market CSV snapshots for all symbols at once — company profiles, EOD prices, earnings surprises, financial statements, ratings, scores, DCF, key metrics, ratios, peers, price targets; output as JSON lines or CSV (`--format csv`)
...
| `bulk` | Full-market bulk CSV snapshots | `profile`, `etf-holder`, `eod`, `earnings-surprises`, `income-statement`, `balance-sheet-statement`, `cash-flow-statement`, `income-statement-growth`, `balance-sheet-statement-growth`, `cash-flow-statement-growth`, `key-metrics-ttm`, `ratios-ttm`, `scores`, `rating`, `upgrades-downgrades-consensus`, `price-target-summary`, `peers`, `dcf` |
...
# Bulk: all company profiles (part 0, NDJSON)
fmp bulk profile --part 0

# Bulk: all 4 profile parts as CSV, rate-limit paced
fmp bulk profile --format csv --all

# Bulk: EOD prices for a date
fmp bulk eod --format csv --date 2024-01-02

# Bulk: all income statements for a year/period
fmp bulk income-statement --format csv --year 2023 --period annual

# Bulk: all ratings for every symbol
fmp bulk rating --format csv

# Bulk: pipe directly into postgres
fmp bulk eod --format csv --date 2024-01-02 | psql -c "COPY eod_prices FROM STDIN WITH CSV HEADER"
```
