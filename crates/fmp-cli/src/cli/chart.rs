...
pub struct EodLightArgs {
  ...
  #[arg(
    long,
    help = "Earliest date to return (YYYY-MM-DD, inclusive; defaults to 5 years ago)"
  )]
...
pub struct IntradayArgs {
  ...
  #[arg(
    long,
    default_value = "1hour",
    help = "Bar interval: 1min, 5min, 15min, 30min, 1hour, 4hour"
  )]
  pub interval: String,
    ...
    let interval = match self.interval.as_str() {
      ...
      _ => {
        return Err(eyre::eyre!(
          "Invalid interval. Use: 1min, 5min, 15min, 30min, 1hour, 4hour"
        ));
      }
    };
...
pub struct TechnicalIndicatorArgs {
  ...
  #[arg(
    long,
    default_value = "20",
    help = "Lookback period in bars (e.g., 14 for RSI, 20 or 50 for SMA/EMA, 14 for ADX)"
  )]
  pub period: u32,
  ...
  #[arg(
    long,
    default_value = "daily",
    help = "Price timeframe: 1min, 5min, 15min, 30min, 1hour, 4hour, daily"
  )]
  pub timeframe: String,

  #[arg(
    long,
    help = "Earliest datetime to return (ISO 8601 / RFC 3339, e.g., 2024-01-01T00:00:00Z)"
  )]
  pub from: Option<String>,

  #[arg(
    long,
    help = "Latest datetime to return (ISO 8601 / RFC 3339, e.g., 2024-12-31T23:59:59Z)"
  )]
  pub to: Option<String>,
}
